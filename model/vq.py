...
"""
Adapted from: https://github.com/facebookresearch/audiocraft/blob/main/audiocraft/quantization/core_vq.py
"""
...
def laplace_smoothing(x: mx.array, n_categories: int, epsilon: float = 1e-5):
...
def sample_vectors(samples: mx.array, num: int):
    num_samples = samples.shape[0]

    if num_samples >= num:
        indices = mx.array(np.random.permutation(num_samples)[:num])
    else:
        indices = mx.random.randint(0, num_samples, (num,))

    return samples[indices]    

def bincount(arr: mx.array, min_length: int):
    result = mx.zeros((min_length))
    for val in arr:
        result[val] += 1
    return result

def kmeans(samples: mx.array, num_clusters: int, num_iters: int = 10):
    dim = samples.shape[-1]

    means = sample_vectors(samples, num_clusters)
    for _ in range(num_iters):
        diffs = mx.expand_dims(samples, axis=1) - mx.expand_dims(means, axis=0)
        dists = -(diffs ** 2).sum(axis=-1)
        dist_dim = dists.shape[-1]
        buckets = dists.argmax(axis=-1)
        num_clusters = max(dist_dim, num_clusters)
        bins = bincount(buckets, num_clusters)
        zero_mask = bins == 0
        bins_min_clamped = mx.where(zero_mask, 1, bins)
        # Accumulate samples into new_means
        new_means = mx.zeros([num_clusters, dim])
        for i in range(samples.shape[0]):
            new_means[buckets[i]] += samples[i]

        # Normalize by bin counts
        new_means /= bins_min_clamped[..., None]
        means = mx.where(zero_mask[..., None], means, new_means)
...
class EuclideanCodebook(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.decay = decay
        init_fn: Union[Callable[..., mx.array], Any] = uniform_init if not kmeans_init else mx.zeros
    ...
    def expire_codes_(self, batch_samples):
        ...
        new_shape = 1
        for d in batch_samples.shape[:-1]:
            new_shape *= d
        batch_samples = batch_samples.reshape((new_shape, batch_samples.shape[-1]))
    ...
    def __call__(self, x):
        ...
        if self.training:
            ...
            self.cluster_size = self.decay * self.cluster_size + (1 - self.decay) * embed_onehot.sum(0)
            embed_sum = mx.matmul(x.T, embed_onehot)
            self.embed_avg = self.decay * self.embed_avg +  (1 - self.decay) * embed_sum.T
            cluster_size = (
                laplace_smoothing(self.cluster_size, self.codebook_size, self.epsilon)
                * self.cluster_size.sum()
            )
            embed_normalized = self.embed_avg / mx.expand_dims(cluster_size, axis=1)
